using Chet.Template.GUI.Domain.Exceptions;
...
namespace Chet.Template.GUI.Domain.Githubs
{
    public class GithubManager : ITransientDependency
    {
        ...
        public async Task<string> GetSourceCodeAsync(string type)
        {
            try
            {
                var repsotiryName = _configuration.GetValue<string>("Github:Chet.Template:RepsotiryName");
                var author = _configuration.GetValue<string>("Github:Chet.Template:Author");
